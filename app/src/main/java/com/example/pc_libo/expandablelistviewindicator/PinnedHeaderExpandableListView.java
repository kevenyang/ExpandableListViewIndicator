...
public class PinnedHeaderExpandableListView extends ExpandableListView implements OnScrollListener,OnGroupClickListener {
    ...
    private static final int GROUP_STATUS_OPEN = 1;
    private static final int GROUP_STATUS_CLOSE = 0;
    ...
    private void headerViewClick() {
        ...
        if (mAdapter.getGroupClickStatus(groupPosition) == GROUP_STATUS_OPEN) {
            this.collapseGroup(groupPosition);
            mAdapter.setGroupClickStatus(groupPosition, GROUP_STATUS_CLOSE);
        }
        else{
            this.expandGroup(groupPosition);
            mAdapter.setGroupClickStatus(groupPosition, GROUP_STATUS_OPEN);
        }
    ...
    public boolean onGroupClick(ExpandableListView parent,View v,int groupPosition,long id) {
        if (mAdapter.getGroupClickStatus(groupPosition) == GROUP_STATUS_CLOSE) {
            mAdapter.setGroupClickStatus(groupPosition, GROUP_STATUS_OPEN);
            parent.expandGroup(groupPosition);
            //Header自动置顶
            //parent.setSelectedGroup(groupPosition);

        } else if (mAdapter.getGroupClickStatus(groupPosition) == GROUP_STATUS_OPEN) {
            mAdapter.setGroupClickStatus(groupPosition, GROUP_STATUS_CLOSE);
